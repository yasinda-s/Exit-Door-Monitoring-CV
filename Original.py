...
def getROIDimensions(exitAreaVertices):
    roi_x1y1, roi_x2y2, roi_x3y3, roi_x4y4 = exitAreaVertices[0],exitAreaVertices[1],exitAreaVertices[2],exitAreaVertices[3]
    roiX, roiY = min(roi_x1y1[0], roi_x2y2[0], roi_x3y3[0], roi_x4y4[0]), min(roi_x1y1[1], roi_x2y2[1], roi_x3y3[1], roi_x4y4[1])
    _, roiHeight = max(roi_x1y1[0], roi_x2y2[0], roi_x3y3[0], roi_x4y4[0]) - roiX, max(roi_x1y1[1], roi_x2y2[1], roi_x3y3[1], roi_x4y4[1]) - roiY 
    return roiY, roiHeight
...
def peopleDetectionYOLO(frame, exitAreaVertices, exitDoorVertices):
    ...
    human_area_count = 0
    human_door_count = 0
    roiDoorY, roiDoorHeight = getROIDimensions(exitDoorVertices)
    roiAreaY, roiAreaHeight = getROIDimensions(exitAreaVertices)
    ...
    for result in results:
        for r in result.boxes.data.tolist():
            ...
            if class_id==0 and score>=detection_threshold:
                    feet_y = y2 - 5
                    if roiAreaY <= feet_y <= roiAreaY + roiAreaHeight:
                        human_area_count += 1
                    if roiDoorY <= feet_y <= roiDoorY + roiDoorHeight:
                        human_door_count += 1
